...
command ls                # 忽略 ls 的任何 alias 直接执行程序或者内建命令
builtin cd                # 忽略 cd 的任何 alias 直接运行内建的 cd 命令
...
cmd1 | cmd2               # 管道，将 cmd1 的标准输出重定向到 cmd2 的标准输入
...
n>&m                      # 文件描述符 n 被作为描述符 m 的一个副本，输出用
n<&m                      # 文件描述符 n 被作为描述符 m 的一个副本，输入用
